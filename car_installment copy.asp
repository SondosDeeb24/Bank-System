...
Dim adoCon, strSQL, button_prssed, email, rsobject, user_balance, update_balance, new_balance
...
button_prssed = Request.Form("payment_method") 'get the value of the button got pressend
...
Set adoCon =  Server.CreateObject("ADODB.Connection")'make access to the database 
...
rsobject.Open strSQL, adoCon ' Open the recordest wiht the SQL query
...
If button_prssed <> "" Then ' it means if the button_prssend was clicked then excute the following

    'update the balance value by deducting the first payment 

    If button_prssed = "plan1" Then
        new_balance = user_balance - amount 

    ElseIf button_prssed = "plan2" Then
        new_balance = user_balance - plan2

    ElseIf button_prssed = "plan3" Then
...
<div class="container">
    ...
    <div class="plan1">
        <h4>Plan 1 </h4>
        <br><br>
        <p>This plan fits the amount of money you perfer to pay monthly </p>
        <br><br>
        <p>If you paid <%= Int(amount)%> thousends every month, you will finish the car's installment in 
            <%= plan1_months%> months
            <% If plan1_weeks > 0 Then %>
                and <%= plan1_weeks %> weeks
            <% End If %>
        </p>    
        
        <!-- <input type="submit" name = "payment_method" value="Start first payment">  -->
        <form method="post" action="car_installment_plans.asp"></form>
            <input type="hidden" name="payment_method" value="plan1">
            <input type="hidden" name="price" value="<%= price %>">
            <input type="hidden" name="salary" value="<%= salary %>">
            <input type="hidden" name="amount" value="<%= amount %>">
            <input type="hidden" name="months" value="<%= months %>">
            <input type="submit" value="Start first payment"><!--NEW -->
        </form>
    </div>
    <!-- Print Plan 2 --------------------------------------------------------------------------->
    <div class="plan2">
        <h4>Plan 2 </h4>
        <br><br>
        <p> This Plan fits the duration you perfer to finish the car's installment during it</p>
        <br><br>
        <p> Here you have to pay <%= Int(plan2)%>  thoused for <%= months%>  months </p>
        <!-- <input type="submit" name = "payment_method" value="Start first payment"> -->
        <form method="post" action="car_installment_plans.asp"></form>
    ...
    <div class="plan3">
        <h4>Plan 3 </h4>
        <br><br>
        <p>This Plan is suggestion from our bank team according to your monthly salary</p>
        <p>If you paid <%= Int(salary*0.5 )%> thousends every month, you will finish the car's installment in 
            <%= plan3_months%> months
            <% If plan3_weeks > 0 Then %>
                and <%= plan3_weeks %> weeks
            <% End If %>
        </p>
        <!-- <input type="submit" name = "payment_method" value="Start first payment"> -->
        <form method="post" action="car_installment_plans.asp"></form>
